...
public class GameRunManager : MonoBehaviour
{
    ...
    [SerializeField] private List<WeaponDef> startingWeapons = new();
    [SerializeField] private List<ArmorDef> startingArmors = new();
    [SerializeField] private List<AccessoryDef> startingAccessories = new();
    ...
    private void InitializeStartingParty()
    {
        ...
        for (int i = 0; i < UnitPrefabs.Count; i++)
        {
            ...
            CurrentParty.Add(new PartyMemberRuntimeData
            {
                ...
                isDead = false,
                weaponPrimary = primary != null ? primary.def : null,
                weaponPrimaryAmmo = primary != null && primary.def != null ? primary.def.magazineSize : 0,
                weaponSecondary = secondary != null ? secondary.def : null,
    ...
    private void InitializeInventory(bool force)
    {
        ...
        if (startingWeapons != null)
        {
            for (int i = 0; i < startingWeapons.Count; i++)
                Inventory.AddWeapon(startingWeapons[i], 1);
        }

        if (startingArmors != null)
        {
            for (int i = 0; i < startingArmors.Count; i++)
                Inventory.AddArmor(startingArmors[i], 1);
        }

        if (startingAccessories != null)
        {
            for (int i = 0; i < startingAccessories.Count; i++)
                Inventory.AddAccessory(startingAccessories[i], 1);
        }
    }

    public bool TryEquipWeapon(string memberId, WeaponDef def, int slotIndex)
    {
        if (string.IsNullOrEmpty(memberId) || def == null) return false;
        ...
        if (!Inventory.HasWeapon(def, 1)) return false;

        var oldDef = (slotIndex == 0) ? member.weaponPrimary : member.weaponSecondary;

        if (!Inventory.RemoveWeapon(def, 1)) return false;

        if (oldDef != null)
            Inventory.AddWeapon(oldDef, 1);

        SetWeaponSlot(member, slotIndex, def, def.magazineSize);
        return true;
    }
    ...
    public bool TryUnequipWeapon(string memberId, int slotIndex)
    {
        ...
        var def = (slotIndex == 0) ? member.weaponPrimary : member.weaponSecondary;
        if (def == null) return false;

        Inventory.AddWeapon(def, 1);
        SetWeaponSlot(member, slotIndex, null, 0);
        return true;
    }

    public bool TryEquipArmor(string memberId, ArmorDef def)
    {
        if (string.IsNullOrEmpty(memberId) || def == null) return false;
        var member = FindPartyMember(memberId);
        if (member == null) return false;

        if (!Inventory.RemoveArmor(def, 1)) return false;

        if (member.armor != null)
            Inventory.AddArmor(member.armor, 1);
    ...
    public bool TryUnequipArmor(string memberId)
    {
        if (string.IsNullOrEmpty(memberId)) return false;
        var member = FindPartyMember(memberId);
        if (member == null || member.armor == null) return false;
    ...
    public bool TryEquipAccessory(string memberId, AccessoryDef def)
    {
        if (string.IsNullOrEmpty(memberId) || def == null) return false;
        var member = FindPartyMember(memberId);
        if (member == null) return false;

        if (!Inventory.RemoveAccessory(def, 1)) return false;

        if (member.accessory != null)
            Inventory.AddAccessory(member.accessory, 1);

        member.accessory = def;
        return true;
    }
    ...
    public bool TryUnequipAccessory(string memberId)
    {
        if (string.IsNullOrEmpty(memberId)) return false;
        var member = FindPartyMember(memberId);
        if (member == null || member.accessory == null) return false;
    ...
    private void SetWeaponSlot(PartyMemberRuntimeData member, int slotIndex, WeaponDef def, int ammo)
    {
        if (member == null) return;
        if (slotIndex == 0)
        {
            member.weaponPrimary = def;
            member.weaponPrimaryAmmo = ammo;
        }
        else
        {
            member.weaponSecondary = def;
