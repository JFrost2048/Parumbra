        ...
        private List<Vector2Int> partySpawnCoords = new()
{
    ...
    new Vector2Int(1, 1),
        ...
        private void ApplyRuntimeDataToSpawnedUnit(Unit unit, PartyMemberRuntimeData data)
        {
            ...
            if (health != null)
                health.SetMaxHP(data.maxHP, data.currentHP);
        ...
        private void ApplyRuntimeEquipmentToSpawnedUnit(Unit unit, PartyMemberRuntimeData data)
        {
            ...
            ApplyWeaponSlot(primary, data.weaponPrimaryItem != null ? data.weaponPrimaryItem.weaponDef : null, data.weaponPrimaryAmmo);
            if (primary != null)
                unit.SetWeaponRuntime(primary);

            var secondary = unit.GetWeaponRuntimeBySlot(1);
            if (maxSlots > 1)
                ApplyWeaponSlot(secondary, data.weaponSecondaryItem != null ? data.weaponSecondaryItem.weaponDef : null, data.weaponSecondaryAmmo);
        ...
        public void SaveCurrentPartyStateToRunManager()
        {
            ...
            for (int i = 0; i < party.Count; i++)
            {
                ...
                if (health != null)
                    member.maxHP = health.MaxHP;

                if (resources != null)
                    member.currentAP = resources.CurrentAP;

                var primary = unit.GetWeaponRuntimeBySlot(0);
                member.weaponPrimaryItem = ResolveWeaponItemFromMember(primary != null ? primary.def : null, member);
                member.weaponPrimaryAmmo = primary != null ? primary.CurrentAmmo : 0;

                if (unit.MaxWeaponSlots > 1)
                {
                    var secondary = unit.GetWeaponRuntimeBySlot(1);
                    member.weaponSecondaryItem = ResolveWeaponItemFromMember(secondary != null ? secondary.def : null, member);
                    member.weaponSecondaryAmmo = secondary != null ? secondary.CurrentAmmo : 0;
                }
                else
                {
                    member.weaponSecondaryItem = null;
        ...
        private ItemData ResolveWeaponItemFromMember(WeaponDef def, PartyMemberRuntimeData member)
        {
            if (def == null || member == null)
                return null;

            if (member.weaponPrimaryItem != null && member.weaponPrimaryItem.weaponDef == def)
                return member.weaponPrimaryItem;

            if (member.weaponSecondaryItem != null && member.weaponSecondaryItem.weaponDef == def)
                return member.weaponSecondaryItem;
